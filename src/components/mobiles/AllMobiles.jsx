...
const fetchData = async () => {
  ...
  const handleKeyPress = async (event) => {
    if (event.key === 'Enter' && searchTerm !=='') {
      ...
      try {
        const response = await fetch(`http://localhost:8005/api/search/?keyword=${searchTerm}`);
        if (!response.ok) {
  ...
  function getProducerById(input, id) {
    ...
    return item ? item.producers : 'Unknown';
}

function getTypeById(input, id) {
  const item = input.find(item => item.id === id);
...
const fetchDataType = async () => {
  ...
  const handleSubmit = async (e) => {
    ...
    axios.put(`http://localhost:8005/api/mobiles/${idEdit}/`, formData, config)
        .then(response => {
            console.log(response);
            handleClose1();
            fetchData();
            alert("Edit mobiles thành công!");
  ...
  return (
    <Box style={{
      ...
      {searchResults ? (
        ...
        handleOpen1={handleOpen1}></CardMobile>
        </Box>
      ))
      ): searchTerm ==='' ? (
      ...
      <CardContent>
        ...
        <Typography variant="body2" color="text.secondary" style={{
          ...
          Producer: {getProducerById(dataBranch, bookDetail.producer )}
        </Typography>

        <Typography variant="body2" color="text.secondary" style={{
          marginBottom: 10
        }}>
          Type: {getTypeById(type, bookDetail.type )}
        </Typography>
      </CardContent>
      {role==="manager" && (
        <CardActions>
